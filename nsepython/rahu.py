...
if mode == "vpn":
    ...
    def nsefetch(payload: str):
        ...
        def refresh_cookies():
            os.popen(
                f'curl -c cookies.txt "https://www.nseindia.com" {curl_headers}'
            ).read()
            os.popen(
        ...
        def get_proxy_list():
            ...
            for proxy_string in proxy_strings:
                ...
                if "@" in proxy_string:
                    ...
                    proxy_list.append(f"http://{proxy_string}")
...
def nse_expirydetails(
    payload, i=0
):  # Can make problem. Use nse_expirydetails_by_symbol()
    expiry_dates = payload["records"]["expiryDates"]
    expiry_dates = [
        datetime.datetime.strptime(date, "%d-%b-%Y").date() for date in expiry_dates
    ]
    expiry_dates = [
        date.strftime("%d-%b-%Y")
        ...
        for item in payload.get("stocks", []):
            if meta in item["metadata"]["instrumentType"]:
                ...
                if optionType in ("Put", "Call"):
                    if (
                        item["metadata"]["expiryDate"] == expiryDate
                        and item["metadata"]["optionType"] == optionType
                        and item["metadata"]["strikePrice"] == strikePrice
                    ):
                        metadata = item["metadata"]
                        break
    else:
...
def nse_preopen_movers(key="FO", filter=1.5):
    ...
    return preOpen_gainer[preOpen_gainer["pChange"] > 1.5], preOpen_gainer[
        preOpen_gainer["pChange"] < -1.5
    ]
